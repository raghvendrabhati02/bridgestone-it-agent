...
generate_esad_v3.py
===================
Bridgestone IT AI Assistant — BST-ESAD-2026-001
Document generator incorporating high-resolution dark-themed enterprise Mermaid PNG diagrams
rendered directly into the Word document (.docx).
...
OUTPUT_PATH = r"C:\Projects\bridgestone-it-agent\BST-ESAD-2026-001_Bridgestone_IT_Agent_v3.docx"
...
def fig_caption(doc, text):
    fig_counter[0] += 1
    p = _new_para(doc, align=WD_ALIGN_PARAGRAPH.CENTER, space_before=4, space_after=12)
...
def embed_diagram(doc, diagram_name, caption_text, width_inches=6.2):
    """
    Embed rendered dark-themed Mermaid PNG image into the Word document.
    Falls back to text block if image not found.
    """
...
def build_esad_doc():
    ...
    body(doc, "The platform routes employee IT issues through a 23-node LangGraph agentic pipeline before escalating to human intervention. The AI agent diagnoses issues, retrieves enterprise knowledge, executes enterprise tooling, performs root-cause analysis and — where self-service resolution is not possible — creates a properly enriched, validated ServiceNow Incident automatically.")
    ...
    body(doc, "The sequence diagram below illustrates the end-to-end flow from an employee submitting an IT issue to resolution or ServiceNow incident creation.")
    embed_diagram(doc, "fig_8_1_issue_resolution", "Employee Issue Resolution — End-to-End Sequence Diagram")

    h2(doc, "8.2 Privileged Software Approval Workflow")
    body(doc, "Requests for privileged software (VS Code, SAP GUI, Adobe CC) follow a structured Manager → Admin dual-approval chain before access is granted.")
    embed_diagram(doc, "fig_8_2_approval_workflow", "Privileged Software Dual-Approval Workflow Sequence")

    h2(doc, "8.3 SLA Escalation State Machine")
    body(doc, "The SLA engine evaluates every open ticket every 60 seconds and progresses it through seven states. Notifications are dispatched at each threshold crossing.")
    embed_diagram(doc, "fig_8_3_sla_state_machine", "SLA Escalation State Machine — Workflow States")

    # ── CH 9: HIGH-LEVEL SYSTEM ARCHITECTURE ──────────────────────────────────
    h1(doc, "High-Level System Architecture", num=9)
    h2(doc, "9.1 Architecture Overview")
    body(doc, "The platform is organised into seven architectural zones. Cross-zone communication occurs strictly through well-defined REST APIs, ORM models, and system adapters.")
    embed_diagram(doc, "fig_9_1_high_level_arch", "High-Level System Architecture — Topology & Component Layers")
    ...
    h2(doc, "10.1 Purpose")
    body(doc, "The AI subsystem is responsible for intent understanding, multi-step diagnostic reasoning, classification and decision making. These functions are kept separate from deterministic system actions.")

    h2(doc, "10.2 AI Provider Fallback Architecture")
    body(doc, "The BaseAIProvider interface decouples callers from concrete AI providers. A three-tier fallback guarantees high availability.")
    embed_diagram(doc, "fig_10_1_ai_fallback", "AI Provider Three-Tier Fallback Architecture")
    ...
    body(doc, "The ClassificationService acts as the boundary between AI output and ServiceNow incident creation.")
    ptable(doc,
           ["Contract", "Mechanism"],
           [
               ["Valid category enforcement",  "LLM output validated against incident_config.json['valid_categories']"],
               ["Confidence thresholds",        "HIGH >= 0.90 | MEDIUM 0.70–0.89 | LOW < 0.70 — clarification requested below threshold"],
               ["Pydantic schema validation",   "All LLM classification responses parsed through ITSMClassification model"],
               ["Deterministic fallback",       "11-domain keyword rules engine activates on LLM error, rate limit or hallucination"],
               ["Assignment group resolution",  "Delegated to ConfigurationService — never resolved by the AI directly"],
               ["Priority / impact / urgency",  "Not produced by AI; computed by IncidentEnrichmentService from business rules"],
           ],
           col_widths_cm=[5.5, 12.5])
    note(doc, "This design prevents AI hallucination from producing invalid ServiceNow field values that would fail API validation or create mis-routed tickets.")

    # ── CH 11: LANGGRAPH WORKFLOW ─────────────────────────────────────────────
    h1(doc, "LangGraph Workflow", num=11)
    h2(doc, "11.1 Purpose")
    body(doc, "LangGraph's StateGraph provides the execution backbone for the AI agent. Nodes represent single-responsibility functions linked by conditional edge logic.")

    h2(doc, "11.2 AgentState — Shared State Schema")
    ptable(doc,
           ["Field Group", "Fields"],
           [
    ...
    h2(doc, "11.3 Full Workflow — Node Graph")
    embed_diagram(doc, "fig_11_1_langgraph_workflow", "LangGraph 23-Node Workflow — Conditional State Graph Topology")

    h2(doc, "11.4 Routing Logic")
    ptable(doc,
    ...
    h2(doc, "11.5 Key Design Decisions")
    ptable(doc,
           ["Decision", "Rationale"],
           [
               ["Multi-step Tool → Multi-Step loop",  "Resolves multi-cause issues in a single turn without repeated user prompting"],
               ["Approval gate before Action node",    "Prevents tool execution affecting enterprise systems without explicit authorisation"],
               ["Ticket Lifecycle as shared node",     "Both ticket creation and standalone paths share one node — avoids duplicated state logic"],
               ["Context Router before Intent",        "Short-circuits the full pipeline for follow-up messages on existing tickets"],
           ],
           col_widths_cm=[6.0, 12.0])

    # ── CH 12: ENTERPRISE INTEGRATION ARCHITECTURE ───────────────────────────
    h1(doc, "Enterprise Integration Architecture", num=12)
    h2(doc, "12.1 Integration Strategy")
    body(doc, "All enterprise integrations implement a common BaseAdapter contract supporting live and mock modes via configuration flags.")

    h2(doc, "12.2 Integration Topology")
    embed_diagram(doc, "fig_12_1_integration_topology", "Enterprise Integration Adapter Topology")

    h2(doc, "12.3 ServiceNow Metadata Validation Pipeline")
    body(doc, "ServiceNowClient handles OAuth2 Password Grant and Refresh Token rotation with thread-safe lock protection.")
    embed_diagram(doc, "fig_12_2_servicenow_validation", "ServiceNow Metadata Validation Pipeline")

    h2(doc, "12.4 ServiceNow Incident Lifecycle")
    ptable(doc,
    ...
    # ── CH 13: SECURITY ARCHITECTURE ──────────────────────────────────────────
    h1(doc, "Security Architecture", num=13)
    h2(doc, "13.1 Security Design Principles")
    body(doc, "The security model enforces defence in depth, least privilege, and complete auditability across all layers.")

    h2(doc, "13.2 Security Stack")
    embed_diagram(doc, "fig_13_1_security_stack", "Security Defence-in-Depth Layered Architecture")

    h2(doc, "13.3 Authentication Flow")
    body(doc, "JWT bearer token issuance and per-endpoint role verification sequence.")
    embed_diagram(doc, "fig_13_2_auth_flow", "Authentication and Authorisation Sequence Flow")

    h2(doc, "13.4 RBAC Model")
    ptable(doc,
    ...
    h2(doc, "13.5 Prompt Injection Protection")
    ptable(doc,
    ...
    h2(doc, "13.6 Immutable Audit Log Design")
    ptable(doc,
           ["Contract", "Implementation"],
           [
               ["Append-only API",           "No update() or delete() methods exist on ImmutableAuditLog — immutability enforced at API level"],
               ["SHA-256 payload hash",      "Payload integrity is verifiable offline against stored hash"],
               ["Correlation ID",            "Every log entry carries request correlation ID from Python contextvars"],
               ["Dual persistence",          "Structured JSON log line emitted AND persisted to database"],
               ["Graceful degradation",      "Database write failure falls back to JSON log only — never raises an exception"],
           ],
           col_widths_cm=[5.5, 12.5])

    # ── CH 14: DATABASE ARCHITECTURE ──────────────────────────────────────────
    h1(doc, "Database Architecture", num=14)
    h2(doc, "14.1 Data Model Overview")
    body(doc, "The platform uses 19 SQLAlchemy ORM models covering ITSM workflow, audit and compliance, and platform operations.")

    h2(doc, "14.2 Core Entity Relationship Summary")
    embed_diagram(doc, "fig_14_1_er_diagram", "Core Database Entity Relationship Diagram")

    h2(doc, "14.3 Core Entities Table")
    ptable(doc,
    ...
    h2(doc, "14.4 Ticket State Machine")
    ptable(doc,
           ["Status", "Description", "Valid Next States"],
           [
               ["NEW",            "Ticket created by AI agent",                 "IN_PROGRESS, WAITING_MANAGER"],
               ["WAITING_MANAGER","Awaiting manager approval",                  "APPROVED, REJECTED"],
               ["APPROVED",       "Manager approved; pending admin action",     "READY_FOR_ADMIN"],
               ["IN_PROGRESS",    "Assigned team working the issue",            "PENDING, RESOLVED"],
               ["PENDING",        "Waiting for user response or more info",     "IN_PROGRESS, AUTO_CLOSED"],
               ["RESOLVED",       "Issue resolved; awaiting user confirmation", "CLOSED, NEW (re-open)"],
               ["CLOSED",         "Final state",                                "—"],
               ["REJECTED",       "Manager rejected request",                   "CLOSED"],
           ],
           col_widths_cm=[4.0, 6.0, 8.0],
           caption_text="Ticket State Machine")

    # ── CH 15: API REQUEST LIFECYCLE ──────────────────────────────────────────
    h1(doc, "API Request Lifecycle", num=15)
    h2(doc, "15.1 Chat Endpoint — End-to-End Sequence")
    body(doc, "The sequence diagram below details the exact middleware chain traversal for an incoming `/chat` request.")
    embed_diagram(doc, "fig_15_1_api_lifecycle", "Chat Endpoint API Request Lifecycle Sequence")

    h2(doc, "15.2 Middleware Failure Responses")
    ptable(doc,
    ...
    # ── CH 16: DEPLOYMENT ARCHITECTURE ────────────────────────────────────────
    h1(doc, "Deployment Architecture", num=16)
    h2(doc, "16.1 Production Topology")
    body(doc, "The platform is containerised via Docker Compose with isolated bridge networking and container health checks.")
    embed_diagram(doc, "fig_16_1_deployment_topology", "Docker Compose Production Deployment Topology")

    h2(doc, "16.2 Environment Matrix")
    ptable(doc,
    ...
    h2(doc, "16.3 Container Health Checks")
    ptable(doc,
           ["Service", "Health Check Command", "Interval", "Timeout", "Retries"],
           [
               ["PostgreSQL", "pg_isready -U prod_admin -d bridgestone_it_agent", "10s", "5s", "5"],
               ["Redis",      "redis-cli ping",                                    "10s", "5s", "5"],
               ["Backend",    "urllib.request.urlopen('http://localhost:8000/health')", "15s", "5s", "3"],
               ["Frontend",   "wget --spider http://localhost:3000/",              "15s", "5s", "3"],
               ["Nginx",      "Depends on backend HEALTHY + frontend HEALTHY",     "—",   "—",  "—"],
           ],
           col_widths_cm=[3.0, 8.0, 2.0, 2.0, 2.0],
           caption_text="Container Health Check Configuration")

    h2(doc, "16.4 Network Isolation")
    ptable(doc,
           ["Port", "Service", "Exposure"],
           [
               ["80",   "Nginx HTTP",          "External (redirects to HTTPS in production)"],
               ["443",  "Nginx HTTPS",          "External"],
               ["9090", "Prometheus",           "Restricted — internal network only"],
               ["3001", "Grafana",              "Restricted — internal network only"],
               ["5432", "PostgreSQL",           "Internal only — not exposed to host network"],
               ["6379", "Redis",                "Internal only — not exposed to host network"],
               ["3000", "Next.js Frontend",     "Internal only — proxied via Nginx"],
               ["8000", "FastAPI Backend",      "Internal only — proxied via Nginx"],
           ],
           col_widths_cm=[2.0, 4.5, 11.5])

    # ── CH 17: MONITORING AND OBSERVABILITY ───────────────────────────────────
    h1(doc, "Monitoring and Observability", num=17)
    h2(doc, "17.1 Observability Architecture")
    body(doc, "The observability stack combines Prometheus metrics, Grafana visualisations, and structured JSON logs linked by correlation IDs.")

    h2(doc, "17.2 Prometheus Metric Categories")
    ptable(doc,
    ...
    h2(doc, "17.3 APScheduler Background Jobs")
    ptable(doc,
    ...
    # ── CH 18: TECHNOLOGY STACK ───────────────────────────────────────────────
    h1(doc, "Technology Stack", num=18)
    h2(doc, "18.1 Backend")
    ptable(doc,
    ...
    # ── CH 19: ADRS ───────────────────────────────────────────────────────────
    h1(doc, "Architecture Decision Records", num=19)
    body(doc, "Architecture Decision Records (ADRs) document technical decisions, context, and consequences.")
    ...
    # ── CH 20: ENGINEERING CHALLENGES ─────────────────────────────────────────
    h1(doc, "Engineering Challenges", num=20)
    challenges = [
        ("20.1 AI Non-Determinism at ITSM Boundaries",
        ...
        ("20.2 Multi-Step Troubleshooting Loop Termination",
        ...
        ("20.3 Concurrent ServiceNow OAuth2 Token Refresh",
        ...
        ("20.4 ServiceNow Metadata Cache Consistency",
        ...
        ("20.5 Approval Workflow State Race Conditions",
        ...
        ("20.6 Prompt Injection Without Over-Blocking",
    ...
    # ── CH 21: FUTURE ROADMAP ─────────────────────────────────────────────────
    h1(doc, "Future Roadmap", num=21)
    h2(doc, "21.1 Near-Term (0–6 Months)")
    ptable(doc,
    ...
    h2(doc, "21.2 Medium-Term (6–18 Months)")
    ptable(doc,
           ["Initiative", "Architectural Impact"],
           [
    ...
    h2(doc, "21.3 Long-Term (18+ Months)")
    ptable(doc,
           ["Initiative", "Architectural Impact"],
           [
    ...
    # ── CH 23: APPENDIX ───────────────────────────────────────────────────────
    h1(doc, "Appendix", num=23)
    h2(doc, "23.1 ServiceNow Field Mapping Reference")
    ptable(doc,
    ...
    h2(doc, "23.2 Environment Variables Reference")
    ptable(doc,
    ...
    h2(doc, "23.3 LangGraph Node Inventory — All 23 Nodes")
    ptable(doc,
    ...
    h2(doc, "23.4 Database Model Inventory — All 19 Models")
    ptable(doc,
           ["#", "Model", "Table", "Domain"],
           [
